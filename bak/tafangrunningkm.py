...
def init_kmbox(ip='192.168.2.188', port='7300', mac='3CC6DC32'):
    """初始化kmboxNet"""
    global kmbox_initialized
    try:
        result = kmNet.init(ip, port, mac)
        if result == 0:
            kmbox_initialized = True
            if logger:
                logger.info(f'kmboxNet初始化成功！IP: {ip}, Port: {port}, MAC: {mac}')
            else:
                print(f'kmboxNet初始化成功！IP: {ip}, Port: {port}, MAC: {mac}')
        else:
            if logger:
                logger.error(f'kmboxNet初始化失败，错误码: {result}')
            else:
                print(f'kmboxNet初始化失败，错误码: {result}')
        return result == 0
    except Exception as e:
        if logger:
            logger.error(f'kmboxNet初始化异常: {str(e)}')
        else:
            print(f'kmboxNet初始化异常: {str(e)}')
        return False
...
def mouse_move(x, y, delay=0.1):
    global kmbox_initialized
    if kmbox_initialized:
        basex, basey = pyautogui.position()
        print(f'当前鼠标位置：({basex}, {basey})')
        kmNet.move_auto(x-basex, y-basey,delay)
def simulate_mouse_wheel(x, y, scroll_amount=120):
    global kmbox_initialized
    if kmbox_initialized:
        mouse_move(x, y)
        kmNet.wheel(scroll_amount)
    else:
...
def press_key(key, press_duration=0.05):
    ...
    key_code = KEY_MAP[key_upper]
    
    if kmbox_initialized:
        kmNet.keydown(key_code)
        time.sleep(adjust_time(press_duration))
...
def run_game_cycle():
    ...
    if not init_kmbox():
        if logger:
            logger.error('kmboxNet初始化失败，无法启动脚本！')
        else:
            print('kmboxNet初始化失败，无法启动脚本！')
        return
